...
import { getComments, addComment } from '../../api/Comments.api';
import { getReelComments, addReelComment } from '../../api/reels.api';
import { ROUTES } from '../../navigation/routes.constants';
...
const CommentsScreen = () => {
  ...
  const { postId, reelId, type = 'post' } = route.params || {};
  const contentId = type === 'reel' ? reelId : postId;
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      let res;
      if (type === 'reel') {
        res = await getReelComments(contentId);
      } else {
        res = await getComments(contentId);
      }
  ...
  ========================= */
  const handleSend = async () => {
    if (!input.trim() || sending || !contentId) return;
    ...
    try {
      ...
      if (type === 'reel') {
        await addReelComment(contentId, commentText);
      } else {
        await addComment(contentId, commentText);
      }
  ...
  /* =========================
     NAVIGATE TO PROFILE
  ========================= */
  const handleUserPress = useCallback((userId, username) => {
    if (!userId && !username) return;

    if (userId === currentUser?._id || username === currentUser?.username) {
      navigation.navigate(ROUTES.MAIN_TAB, { screen: 'PROFILE' });
    } else {
      navigation.navigate(ROUTES.USER_PROFILE, { userId, username });
    }
  }, [navigation, currentUser]);
  ...
  const renderItem = ({ item }) => (
    <View style={styles.commentRow}>
      <TouchableOpacity 
        onPress={() => handleUserPress(item?.user?._id, item?.user?.username)} 
        activeOpacity={0.7}
      >
        <Image
          source={{ uri: item?.user?.profilePicture || 'https://via.placeholder.com/40' }}
